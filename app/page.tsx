...
import { emailService, GeneralStats, ProcessedEmail } from '@/lib/services/emailService';
import { statusService, SystemStatus } from '@/lib/services/statusService';
        ...
        <div className="flex items-center justify-between mb-8 animate-slideIn">
          <div>
            ...
            <p className="text-gray-600 dark:text-gray-400">
              Sistema inteligente de análise de emails
            </p>
          </div>
          <button
            onClick={handleRefresh}
            disabled={refreshing}
            className="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-smooth disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2"
          >
            <svg
              className={`w-5 h-5 ${refreshing ? 'animate-spin-slow' : ''}`}
              fill="none"
              stroke="currentColor"
              viewBox="0 0 24 24"
            >
        ...
        {systemStatus && (
          <div className="mb-8 bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-6 animate-fadeIn">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              <StatusBadge 
                status={systemStatus.listener?.running || systemStatus.emailListener?.running || false} 
              ...
              <StatusBadge 
                status={systemStatus.database?.connected || false} 
                label="Database" 
              />
            </div>
          </div>
